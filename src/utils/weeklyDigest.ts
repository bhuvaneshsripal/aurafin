...
export function buildWeeklyDigestPayload(
  ...
  const topHoldings =
    [...resolved]
      ...
      .map(({ asset, r }) => {
        ...
        return `${asset.name} \u2014 ${formatCurrency(r.value)}${pct}`;
      })
      .join('\n') || 'No holdings yet';
  ...
  const insights = [
    ...
    best ? `${best.asset.name} is your top performer, ${formatPercent(best.r.pnlPercent!)} overall.` : null,
    totalLiabilities > 0
  ...
  return {
    ...
    bestPerformer: best ? `${best.asset.name} (${formatPercent(best.r.pnlPercent!)})` : 'N/A',
    worstPerformer: worst ? `${worst.asset.name} (${formatPercent(worst.r.pnlPercent!)})` : 'N/A',
